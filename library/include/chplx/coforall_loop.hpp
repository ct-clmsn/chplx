...
void coforall(hpx::source_location const &location, Tuple<Ts...> &t, F &&f,
  ...
  detail::coforall(
      t,
      hpx::annotated_function(std::forward<F>(f),
                              detail::generate_annotation(location)),
      std::forward<Args>(args)...);
}
...
void coforall(hpx::source_location const &location,
              Range<T, BoundedType, Stridable> const &r, F &&f,
              Args &&...args) {
...
void coforall(hpx::source_location const &location, Array<T, Domain> const &a,
              F &&f, Args &&...args) {
