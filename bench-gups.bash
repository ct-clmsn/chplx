...
build_chplx_rel_flag() {
    ...
    cd build
    rm -rf *
    chpl "$CURRENT_DIR"/publications/fall2023/gups.chpl -o "$CURRENT_DIR"/build/gups_chapel >/dev/null
    ...
    mkdir -p gups_chplx
    ./backend/chplx -f ../backend/test/gups_cpp.chpl -o gups_chplx
    cd gups_chplx
    mkdir -p build
    cd build
...
doit_hpx() {
    ...
    for i in {4..64}; do
        ...
        for j in {1..5}; do
            local opt=$($_prefix_ $CURRENT_DIR/build/gups_chplx/build/gups_cpp --hpx:threads=$i)
            #echo $opt
            sum=$(calc "$sum + $(echo $opt | awk -F',' '{print $2}')")
            #echo "$opt" >"$APEX_STORE_DIR/$_dir_meta_name_/proc$i"/runlog
        done
        local _procs_=$(echo $opt | awk -F',' '{print $1}')
...
doit_chapel() {
    ...
    for i in {4..64}; do
        sum=0.000000
        for j in {1..5}; do
            local opt=$(CHPL_RT_NUM_THREADS_PER_LOCALE_QUIET=yes MAX_LOGICAL=$i MAX_PHYSICAL=$i CHPL_RT_NUM_THREADS_PER_LOCALE=$i "$CURRENT_DIR"/build/gups_chapel)
            #echo $opt
            sum=$(calc "$sum + $(echo $opt | awk -F',' '{print $2}')")
        done
        local _procs_=$(echo $opt | awk -F',' '{print $1}')
