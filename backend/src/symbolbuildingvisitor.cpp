...
SymbolBuildingVisitor::SymbolBuildingVisitor(
    chpl::uast::BuilderResult const &chapelBr, std::string const &cfps)
    : br(chapelBr), indent(0), chplFilePathStr(cfps), symstack(), sym(),
      symnode(), symbolTable() {
  ...
  addSymbolEntry(
      "nil",
      Symbol{{nil_kind{}, std::string{"nil"}, {}, symbolTable.symbolTableCount},
             {}});
  addSymbolEntry(
      "bool",
      Symbol{
          {bool_kind{}, std::string{"bool"}, {}, symbolTable.symbolTableCount},
          {}});
  addSymbolEntry("string", Symbol{{string_kind{},
                                   std::string{"string"},
                                   {},
  ...
  addSymbolEntry(
      "int",
      Symbol{{int_kind{}, std::string{"int"}, {}, symbolTable.symbolTableCount},
             {}});
  addSymbolEntry(
      "real",
      Symbol{
          {real_kind{}, std::string{"real"}, {}, symbolTable.symbolTableCount},
          {}});
  addSymbolEntry(
      "byte",
      Symbol{
          {byte_kind{}, std::string{"byte"}, {}, symbolTable.symbolTableCount},
          {}});
  addSymbolEntry("complex", Symbol{{complex_kind{},
                                    std::string{"complex"},
                                    {},
                                    symbolTable.symbolTableCount},
                                   {}});
  addSymbolEntry("range", Symbol{{range_kind{},
                                  std::string{"range"},
                                  {},
                                  symbolTable.symbolTableCount},
                                 {}});
  addSymbolEntry("domain", Symbol{{domain_kind{},
                                   std::string{"domain"},
                                   {},
                                   symbolTable.symbolTableCount},
                                  {}});
  addSymbolEntry(
      "?",
      Symbol{
          {template_kind{}, std::string{"?"}, {}, symbolTable.symbolTableCount},
          {}});

  addSymbolEntry("inlinecxx", Symbol{{std::make_shared<cxxfunc_kind>(
                                          cxxfunc_kind{{{}, "inlinecxx", {}}}),
                                      std::string{"inlinecxx"},
                                      {},
                                      symbolTable.symbolTableCount},
                                     {}});
}
...
bool SymbolBuildingVisitor::enter(const uast::AstNode *ast) {
  // std::cout << "node tag\t" << ast->tag() << '\t' << tagToString(ast->tag())
  // << std::endl;
  switch (ast->tag()) {
    ...
    if (sym && sym->get().kind.has_value()) {
      if (std::holds_alternative<std::shared_ptr<array_kind>>(
        ...
        std::get<std::shared_ptr<array_kind>>(*(sym->get().kind))
            ->dom->ranges.push_back(range_kind{});
      }
    } else {
      symstack.emplace_back(
          Symbol{{std::make_optional<kind_types>(range_kind{}),
    ...
    if (sym && sym->get().kind.has_value() && 0 < sym->get().kind->index()) {
      ...
      if (std::holds_alternative<std::shared_ptr<array_kind>>(
        ...
        if (!sym->get().literal && symref->dom) {
          ...
          std::get<std::shared_ptr<kind_node_type>>(symref->kind)
        ...
        range_kind &symref = std::get<range_kind>(*sym->get().kind);
  ...
  case asttags::Formal: {
    if (sym.has_value() && sym->get().kind.has_value() &&
      ...
      fk->args.emplace_back(
          Symbol{{std::make_optional<kind_types>(), ident, {}, 0}});
  ...
  case asttags::Block: {
    if (sym && sym->get().kind.has_value()) {
      if (std::holds_alternative<std::shared_ptr<func_kind>>(
        ...
        std::shared_ptr<func_kind> &fk =
            std::get<std::shared_ptr<func_kind>>(*(sym->get().kind));
        fk->args.emplace_back(Symbol{
            {nil_kind{}, std::string{"nil"}, {}, symbolTable.symbolTableCount},
            {}});
      }
    }
  } break;
  ...
  case asttags::Function: {
    ...
    if (!(sym && sym->get().kind.has_value() && 0 < sym->get().kind->index())) {
      ...
      ast->traverse(v);
      //Function const *fn = dynamic_cast<Function const *>(ast);

      symstack.emplace_back(Symbol{
          {std::optional<kind_types>{std::make_shared<func_kind>(
               func_kind{{symbolTable.symbolTableCount - 1, {}, {}, {}}})},
           v.lookup,
           {},
           0}});
...
void SymbolBuildingVisitor::exit(const uast::AstNode *ast) {
  // std::cout << "exit\t" << ast->tag() << '\t' << tagToString(ast->tag()) <<
  // std::endl;
  switch (ast->tag()) {
  case asttags::AnonFormal:
    break;
  ...
  case asttags::Array: {
    if (sym.has_value()) {
      std::get<std::shared_ptr<kind_node_type>>(
          std::get<std::shared_ptr<array_kind>>(*sym->get().kind)->kind)
          ->children.emplace_back(kind_node_term_type{});
    }
  } break;
  case asttags::Attributes:
    break;
