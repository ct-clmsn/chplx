...
SymbolTable::SymbolTable()
    : symbolTableCount(0), symbolTableRef(), lut(), parentSymbolTableId(0) {
  symbolTableRoot =
      std::make_shared<SymbolTableNode>(SymbolTableNode{0, {}, {}, {}});
...
std::size_t SymbolTable::pushScope() {
  ...
  //
  symbolTableRef->children.push_back(std::make_shared<SymbolTableNode>(
      SymbolTableNode{symbolTableCount, {}, {}, symbolTableRef}));
  ++symbolTableCount;

  symbolTableRef = std::get<std::shared_ptr<SymbolTableNode>>(
      symbolTableRef->children.back());
  lut.push_back(symbolTableRef);
...
void SymbolTable::popScope() {
  if (symbolTableRef->parent &&
      std::holds_alternative<std::shared_ptr<SymbolTableNode>>(
          *symbolTableRef->parent)) {
    symbolTableRef =
        std::get<std::shared_ptr<SymbolTableNode>>(*symbolTableRef->parent);
  }
}

void SymbolTable::addEntry(const std::size_t lutid, std::string const &ident,
                           Symbol s) {
...
bool SymbolTable::findImpl(
    std::shared_ptr<SymbolTableNode> &stref, std::string const &ident,
  ...
  return findImpl(std::get<std::shared_ptr<SymbolTableNode>>(*(stref->parent)),
                  ident, ret);
}

std::optional<Symbol> SymbolTable::find(std::string const &ident) {
  std::shared_ptr<SymbolTableNode> &stref = symbolTableRef;
  std::unordered_map<std::string, Symbol>::iterator entry =
      stref->entries.find(ident);
  ...
  if (stref->parent) {
    const bool found =
        findImpl(std::get<std::shared_ptr<SymbolTableNode>>(*stref->parent),
                 ident, entry);
    if (found) {
      return entry->second;
    }
...
std::optional<Symbol> SymbolTable::find(const std::size_t idx,
...
void SymbolTable::find(const std::size_t idx, std::string const &ident,
