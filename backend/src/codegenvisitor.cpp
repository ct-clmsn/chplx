...
struct StatementVisitor {
   ...
   void operator()(std::shared_ptr<ModuleDeclarationExpression> const& node) {
      ...
      std::vector<std::size_t> main_scoped_statements{};
      {
         ...
         cppv.emitIndent();
         cppofs << "struct __thisModule *_this = nullptr;" << std::endl << std::endl;
         cppv.emitIndent();
         cppofs << "struct __thisModule {" << std::endl << std::endl;
      ...
      cppofs << "void __main() {" << std::endl << std::endl;
      ...
      cppofs << '}' << std::endl << std::endl;
      cppv.emitIndent();
      cppofs << "static void __construct() { __thisModule = new __thisModule(); }" << std::endl << std::endl;
      cppv.emitIndent();
      cppofs << "static void __destruct() noexcept {" << std::endl;
      cppv.emitIndent(); cppv.emitIndent();
      cppofs << "auto *m = __thisModule;" << std::endl;
      cppv.emitIndent(); cppv.emitIndent();
      cppofs << "__thisModule = nullptr;" << std::endl;
      cppv.emitIndent(); cppv.emitIndent();
      cppofs << "delete m;" << std::endl;
      cppv.emitIndent();
      cppofs << "}" << std::endl << std::endl;
      cppv.emitIndent();
      cppofs << "}; // end of module struct" << std::endl;
...
static void generateSourceHeader(std::vector<Symbol> & cfgVars, std::ostream & fos, std::string const& fps, std::string const& chplFile) {
   ...
   {        
      ...
      fos << "#include \"" << prefix << "_driver.hpp\"" << std::endl;
   }        
   ...
   if(0 < cfgVars.size()) {
      ...
      for(auto & opt : cfgVars) {
         ...
         VisitQualifierSuffix(fos, opt.kindqualifier);
         fos << ' ' << opt.identifier;
...
static void generateSourceFooter(std::string const& modStr, std::vector<Symbol> & cfgVars, std::ostream & fos) {
   fos << std::endl
   ...
   if(0 < cfgVars.size()) {
      ...
      for(auto & opt : cfgVars) {
         ...
         std::visit(v, opt.kind);
         fos << ">(&" << "config_" << opt.identifier << "), " << "\"config ";
...
static void generateHpxMain(std::ostream & fos, std::string const& moduleStr)  {
   fos << std::endl
       ...
       << "    " << moduleStr << "::__thisModule->__main();" << std::endl
...
void CodegenVisitor::visit() {
   ...
   std::string prefix = chplFilePathStr.substr(0, pos);
   {
      ...
      generateApplicationHeader(cppFilePathStr, prefix, ofs);
   }

   {
      std::fstream ofs(prefix + "_driver.cpp", std::ios_base::out);
      generateSourceHeader(cfgVars, ofs, cppFilePathStr, chplFilePathStr);
      generateHpxMain(ofs, prefix);
      generateSourceFooter(prefix, cfgVars, ofs);
   }
