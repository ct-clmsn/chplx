// This program file was generated by the chplx compiler.
// The original Chapel program file can be found here: heat.chpl
//
#include "heat.hpp"

namespace heat {

    struct __thisModule* __this = nullptr;

#line 26 "heat.chpl"
    void __thisModule::update(std::vector<double>& d,
        std::vector<double>& d2)
    {
#line 27 "heat.chpl"
        auto NX = nx + 1;
#line 28 "heat.chpl"
        long temp = 0;
        auto irange = hpx::util::detail::make_counting_shape(temp,NX-1);
        hpx::for_each(hpx::execution::par, hpx::util::begin(irange), hpx::util::end(irange), [&](auto i) {
#line 30 "heat.chpl"
        auto idx = ((i-1)>0)?i-1:0;
        auto idx_capped = (1 + i > NX-1)?NX-1:1+i;
            d2[i] = d[i] +
                (((dt * k) / (dx * dx)) * ((d[idx_capped] + d[idx]) - (2 * d[i])));
        });
#line 32 "heat.chpl"
        d2[0] = d2[NX-1];
#line 33 "heat.chpl"
        d2[NX-1] = d2[1];
    };

#line 17 "heat.chpl"
    std::int64_t ghosts = 1;
#line 18 "heat.chpl"
    double k = 0.400000;
#line 19 "heat.chpl"
    double dt = 1.000000;
#line 20 "heat.chpl"
    double dx = 1.000000;
#line 22 "heat.chpl"
    std::int64_t nx = 1000000;
#line 23 "heat.chpl"
    std::int64_t nt = 100;
#line 24 "heat.chpl"
    std::int64_t threads = 20;

    void __thisModule::__main()
    {
#line 38 "heat.chpl"
        auto NX = nx + 1;
#line 40 "heat.chpl"
        // chplx::Array<double, chplx::Domain<1>> data(chplx::Range(0, NX));
        std::vector<double> data(NX);
        for (long i=0;i<NX+1;i++){
            data[i] = i;
        }

#line 41 "heat.chpl"
        // chplx::Array<double, chplx::Domain<1>> data2(chplx::Range(0, NX));
        std::vector<double> data2(NX);
        for(long i=0;i<NX+1;i++){
            data2[i]=i;
        }
#line 43 "heat.chpl"
        auto irange = hpx::util::detail::make_counting_shape(NX);
        hpx::for_each(hpx::execution::par, hpx::util::begin(irange), hpx::util::end(irange), [&](auto i) {
#line 44 "heat.chpl"
            data[i] = 1 + (((i - 1) + nx) % nx);
#line 45 "heat.chpl"
            data2[i] = 0;
        });
#line 52 "heat.chpl"
long tp = 1;
        auto irange2 = hpx::util::detail::make_counting_shape(tp,nt);
        hpx::chrono::high_resolution_timer t;
#line 54 "heat.chpl"
        hpx::for_each(hpx::execution::par, hpx::util::begin(irange2), hpx::util::end(irange2), [&](auto m) {
#line 55 "heat.chpl"
            update(data, data2);
        });
#line 67 "heat.chpl"
        const auto elapsed = t.elapsed();
#line 68 "heat.chpl"
        std::cout << "chapelng," << nx << "," << nt << ","
                  << hpx::resource::get_num_threads() << "," << dt << "," << dx
                  << "," << elapsed << ",0";
    }

}    // namespace heat